 ...
 * Convert MathML element to Markdown math notation
 */
function convertMathML(node: any): void {
  // Determine if this is inline or block math
  const isDisplay = node.properties && 
                    node.properties.display === 'block';
 ...
 */
function convertScriptMath(node: any): void {
  // Extract the math content
  let mathContent = '';
  ...
  const isDisplay = node.properties.type.includes('display');
 ...
 */
function convertClassedMath(node: any): void {
  // Extract the math content
  const mathContent = extractTextContent(node);
  
  // Determine if this is display math
  const isDisplay = Array.isArray(node.properties.className) && 
 ...
 */
function extractMathContent(node: any): string {
