...
import * as relationship from './ast/relationship.js';
export { ast, builder, normalizeAst, relationship };
...
import { defaultRule } from './rules/default-rule.js';
export { TagRule, RuleContext, createRuleContext, RuleRegistry, allRules, createRuleMap, defaultRuleMap, defaultRule };
...
export function renderAstToMarkdown(ast: ast.ASTNode[], options?: RenderOptions): string {
  ...
  if (!relationship.verifyRelationships(ast)) {
    relationship.fixRelationships(ast);
  }
